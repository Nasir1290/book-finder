...
import allBooksData from "../data/bookData.json";
import starSvg from "../assets/star.svg";

export default function BookList({ searchWith, sortingType }) {
  let allData = [...allBooksData];
  const [booksData, setBooksData] = useState(allData);
  // initialize sorted book list
  let sortedBooks = [];

  // sorting logic ascending name order 👇
  if (sortingType === "name_asc") {
    sortedBooks = booksData.sort((a, b) => {
      const titleA = a.title.toUpperCase();
      const titleB = b.title.toUpperCase();
      // conditionally return value depends on titleA and titleB
      return titleA > titleB ? 1 : titleA < titleB ? -1 : 0;
    });
  }

  // sorting logic for descending name order 👇
  ...
  // sorting logic for ascending year order 👇
  if (sortingType === "year_asc") {
    sortedBooks = booksData.sort((a, b) => a.year - b.year);
  }

  // sorting logic for descending year order 👇
  if (sortingType === "year_desc") {
    sortedBooks = booksData.sort((a, b) => b.year - a.year);
  }

  // handle isFavourite button 👇
  const handleIsFavourite = (index) => {
      let updatedBooksData = [...booksData];
      updatedBooksData[index].isFavourite = ! updatedBooksData[index].isFavourite;
      setBooksData(updatedBooksData)
  };

  // initialize filtered books , then assign value depending on condition
  let filteredBooks;
  // if sorting is not defined then it will render default book list
  if (sortingType) {
    filteredBooks = sortedBooks.filter((book) =>
      book.title.toLowerCase().includes(searchWith.toLowerCase())
    );

    // if sorting is defined then it will render sorting book list
  } else {
        ...
        {filteredBooks.map((book, index) => (
          <div key={index} className="space-y-3">
            ...
            <div className="flex items-center justify-center rounded-md border border-[#324251]/30 bg-white p-4">
              <img className="max-w-[144px]" src={bookPic} alt="book name" />
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3 text-xs lg:text-sm">
                ...
                {!book.isFavourite ? (
                  // normal favourite button sty
                  <button
                    onClick={() => handleIsFavourite(index)}
                  ...
                  // design after adding to favourite
                  <button
                    onClick={() => handleIsFavourite(index)}
